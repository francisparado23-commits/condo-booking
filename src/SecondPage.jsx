...
import { db } from './firebase'; // ← connect to your Firebase
...
export default function SecondPage() {
  ...
  const baseUrl = import.meta.env.BASE_URL; // ✅ Base URL defined once
  ...
  const fixImagePaths = (data) => {
    ...
    Object.keys(data).forEach(category => {
      fixed[category] = data[category].map(item => ({
        ...item,
        image: item.image
          ? `${baseUrl}images/${item.image.replace(/^.*[\\/]images[\\/]/, '').replace(/^images\//, '').replace(/^\//, '')}`
  ...
  // ✅ Load food data with correct paths
  const [foodData, setFoodData] = useState(() => {
    const saved = localStorage.getItem('foodData');
    if (saved) {
      try {
        return fixImagePaths(JSON.parse(saved));
      } catch (e) {
        console.error("Invalid foodData:", e);
      }
    }
    // Default data: only filenames
    return fixImagePaths({
      "Hot Dish": [
        { name: "Sopas", price: "₱30", image: "sopas.jpg", addOns: [] },
        { name: "Lugaw", price: "₱30", image: "lugaw.jpg", addOns: [] },
        { name: "Sotanghon", price: "₱30", image: "sotanghon.jpg", addOns: [] },
        { name: "Champorado", price: "₱30", image: "champorado.jpg", addOns: [] }
      ],
      "Pasta": [
        { name: "Spaghetti", price: "₱50", image: "spaghetti.jpg", addOns: [{ name: "More Cheese", price: "₱10" }] },
        { name: "Carbonara", price: "₱50", image: "carbonara.jpg", addOns: [{ name: "More Sauce", price: "₱10" }] }
      ],
      "Rice Meal": [
        { name: "Menudo with Rice", price: "₱90", image: "menudo.jpg", addOns: [{ name: "Extra Rice", price: "₱15" }] },
        { name: "Bicol Express with Rice", price: "₱85", image: "bicol-express.jpg", addOns: [{ name: "Extra Rice", price: "₱15" }] },
        { name: "Sinigang with Rice", price: "₱70", image: "sinigang.jpg", addOns: [{ name: "Extra Rice", price: "₱15" }] }
      ],
      "Drinks": [
        { name: "Hot coffee", price: "₱20", image: "hot-coffee.jpg", addOns: [] },
        { name: "Ice Coffee", price: "₱30", image: "ice-coffee.jpg", addOns: [] },
        { name: "Bottled Water", price: "₱10", image: "water.jpg", addOns: [] },
        { name: "Coke", price: "₱25", image: "coke.jpg", addOns: [] },
        { name: "Mountain Dew", price: "₱25", image: "mountain-dew.jpg", addOns: [] },
        { name: "C2 Apple", price: "₱30", image: "c2-apple.jpg", addOns: [] }
      ],
      "Dessert": [
        { name: "Ice Cream", price: "₱40", image: "ice-cream.jpg", addOns: [] }
      ],
      "Other": [
        { name: "Nova", price: "₱12", image: "nova.jpg", addOns: [] },
        { name: "Piatos", price: "₱12", image: "piatos.jpg", addOns: [] },
        { name: "Sponge", price: "₱12", image: "sponge.jpg", addOns: [] },
        { name: "Cloud 9", price: "₱13", image: "cloud9.jpg", addOns: [] }
      ]
    });
  });
    ...
    const updateFromStorage = () => {
      ...
      if (newData) {
        try {
          ...
          setFoodData(fixImagePaths(parsed));
        } catch (e) {
          console.error("Error updating foodData:", e);
        }
      }
  ...
  useEffect(() => {
    const handleClickAnywhere = (e) => {
      if (showReceipt && receiptRef.current && !receiptRef.current.contains(e.target)) {
  ...
  const addToCart = (item, addOn = null, buttonRef = null) => {
    ...
    if (!addOn) {
      setCart(prev => {
        ...
        if (allUnpairedAddons.length > 0) attachAddon = remainingAddons.shift();
        ...
        const newBase = {
          ...
          image: item.image,
          addOns: attachAddon ? [{ id: attachAddon.uniqueId, name: attachAddon.name.replace('+ ', ''), price: attachAddon.price }] : []
        };
  ...
  const getGroupedCart = () => {
    ...
    cart.forEach(item => {
      ...
      if (!groups[key]) groups[key] = { ...item, qty: 1 };
      else groups[key].qty += 1;
    });
      ...
      {showQR && (
        <div style={styles.modalOverlay}>
          <div style={styles.qrModalBox}>
            ...
            <img src={`${baseUrl}images/Gcash.jpg`} alt="QR Code" style={styles.qrImage} />
                ...
                <div key={idx} style={{ ...styles.foodCard, opacity: available ? 1 : 0.5 }}>
                  <div style={{ position: 'relative', width: '100%', height: '180px', overflow: 'hidden' }}>
                    ...
                    <img
                      ...
                      onError={(e) => {
                        e.target.src = `${baseUrl}images/placeholder.png`;
...
// ✅ Styles arranged in order: Container → Header → Content → Modals → Receipt
const styles = {
  // 📦 MAIN CONTAINER — EXACT SAME RATIO, SCALES TO FIT
  container: {
    fontFamily: 'Segoe UI, Roboto, sans-serif',
    width: '100%',
    minHeight: '100vh',
    backgroundImage: `linear-gradient(rgba(0,0,0,0.55), rgba(0,0,0,0.55)), url(${import.meta.env.BASE_URL}images/Second.gif)`,
    ...
    // Keeps original layout, just scales down
    maxWidth: '1200px',
    margin: '0 auto'
  },

  // 🎛️ HEADER — EXACT SAME SIZE & POSITION
  header: {
  ...
  menuTag: { 
    margin: 0, 
    fontSize: '2rem' // EXACT same size as before
  },
  viewCartBtn: { 
    background: 'white', 
    color: '#d32f2f', 
  ...
  // 🗂️ MAIN LAYOUT — EXACT SAME STRUCTURE
  mainLayout: { 
    display: 'flex', 
  ...
  // 📑 CATEGORY AREA — EXACT SAME WIDTH & STYLE
  categoryScroll: { 
    width: '220px',
    height: '80%', 
    alignSelf: 'center', 
  ...
  // 🍽️ FOOD DISPLAY — EXACT SAME GRID & SIZES
  foodDisplay: { 
    flex: 1, 
    padding: '2rem', 
    overflowY: 'auto' 
  ...
  // 🛒 CART POPUP — EXACT SAME SIZE & POSITION
  cartPopup: { 
    position: 'absolute', 
    top: '100px', 
    right: '20px', 
  ...
  // ✅ SCALE WHOLE PAGE ON SMALL SCREENS — NO LAYOUT CHANGES
